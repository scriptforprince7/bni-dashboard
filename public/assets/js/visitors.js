    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            try {
                ...
                const response = await fetch('http://backend.bninewdelhi.com/api/update-visitor', {
                    method: 'PUT',
                    headers: {
        ...
        async function renderVisitors(visitorsToShow) {
            ...
            const tableContent = await Promise.all(filteredVisitors.map(async (visitor, index) => {
                ...
                const createDocumentDisplay = (imgPath, docNumber, docType, visitorId) => {
                    ...
                    return `
                        <div class="doc-container">
                            <a href="http://backend.bninewdelhi.com/api/uploads/${folderName}/${imgPath}" target="_blank">
                                <img src="http://backend.bninewdelhi.com/api/uploads/${folderName}/${imgPath}" alt="${docType}" style="max-width: 100px;">
                            </a>
...
async function handleInductionKitApproval(visitorId, chapterId) {
    ...
    try {
        ...
        const response = await fetch('http://backend.bninewdelhi.com/api/getRequestedChapterRequisition');
        ...
        const updateResponse = await fetch('http://backend.bninewdelhi.com/api/update-visitor', {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
