...
const populateMembershipDropdown = async () => {
  try {
    ...
    const uniqueTypes = [
      ...new Set(
        memberships.map((membership) => membership.member_current_membership)
      ),
    ];
...
const populateStatusDropdown = async () => {
  try {
    ...
    const response = await fetch(window.BNI.endpoints.members);
    if (!response.ok)
      throw new Error("Error fetching member current membership");
...
const updateDropdownText = (dropdown, selectedValue) => {
  const selectedItem = dropdown.querySelector(
    `.dropdown-item[data-value="${selectedValue}"]`
  );
  const dropdownToggle = dropdown
    .closest(".dropdown")
    .querySelector(".dropdown-toggle");
  ...
  if (selectedItem && dropdownToggle) {
    console.log(
      `Updating dropdown: ${dropdown.id}, selectedValue: ${selectedValue}`
    ); // Debugging line
...
document.getElementById("apply-filters-btn").addEventListener("click", () => {
  const regionId =
    window.BNI.elements.dropdowns.region
      .querySelector(".dropdown-item.active")
      ?.getAttribute("data-value") || "";
  const chapterId =
    window.BNI.elements.dropdowns.chapter
      .querySelector(".dropdown-item.active")
      ?.getAttribute("data-value") || "";
  const membershipYear =
    window.BNI.elements.dropdowns.membership
      .querySelector(".dropdown-item.active")
      ?.getAttribute("data-value") || "";
  const categoryId =
    window.BNI.elements.dropdowns.category
      .querySelector(".dropdown-item.active")
      ?.getAttribute("data-value") || "";
  const accoladesId =
    window.BNI.elements.dropdowns.accolades
      .querySelector(".dropdown-item.active")
      ?.getAttribute("data-value") || "";
  const status =
    window.BNI.elements.dropdowns.status
      .querySelector(".dropdown-item.active")
      ?.getAttribute("data-value") || "";
...
window.addEventListener("load", () => {
  ...
  if (regionId)
    updateDropdownText(window.BNI.elements.dropdowns.region, regionId);
  if (chapterId)
    updateDropdownText(window.BNI.elements.dropdowns.chapter, chapterId);
  if (membershipYear)
    updateDropdownText(
      window.BNI.elements.dropdowns.membership,
      membershipYear
    ); // Ensure chapter type is uppercased
  if (categoryId)
    updateDropdownText(window.BNI.elements.dropdowns.category, categoryId); // Ensure chapter status is uppercased
  if (accoladesId)
    updateDropdownText(window.BNI.elements.dropdowns.accolades, accoladesId); // Ensure chapter status is uppercased
  ...
  try {
    ...
    window.BNI.state.filteredMembers = window.BNI.state.allMembers.filter(
      (member) => {
        return (
          (!filters.regionId ||
            member.region_id === parseInt(filters.regionId)) &&
          (!filters.chapterId ||
            member.chapter_id === parseInt(filters.chapterId)) &&
          (!filters.membershipYear ||
            member.member_current_membership === filters.membershipYear) &&
          (!filters.categoryId ||
            member.category_id === parseInt(filters.categoryId)) &&
          (!filters.accoladesId ||
            (Array.isArray(member.accolades_id) &&
              member.accolades_id.includes(parseInt(filters.accoladesId)))) &&
          (!filters.status ||
            member.member_status.toUpperCase() === filters.status)
        );
      }
    );
    ...
    displayMembers(
      window.BNI.state.filteredMembers.slice(0, window.BNI.state.entriesPerPage)
    );
...
function displayMembers(members) {
  ...
  members.forEach((member, index) => {
    const fullName = `${member.member_first_name} ${
      member.member_last_name || ""
    }`;
    const formattedDate = member.member_induction_date
      ? member.member_induction_date.substring(0, 10)
      : "N/A";
    const chapterName =
      window.BNI.state.chaptersMap[member.chapter_id] || "N/A";
    ...
    row.innerHTML = `
            <td>${
              (window.BNI.state.currentPage - 1) *
                window.BNI.state.entriesPerPage +
              index +
              1
            }</td>
            <td style="border: 1px solid grey;">
                <div class="d-flex align-items-center">
                    ...
                    <a href="javascript:void(0);" class="member-name" data-member-id="${
                      member.member_id
                    }" data-member-email="${
      member.member_email_address
            ...
            <td style="border: 1px solid grey;">
                <span class="badge bg-${
                  member.member_status === "active" ? "success" : "danger"
                }">
            ...
            <span class="badge bg-info text-light" style="cursor:pointer; color:white; margin-right: 4px;">
                    <a href="/m/view-member/?member_id=${
                      member.member_id
                    }" style="color:white">View</a>
                </span>
                <span class="badge bg-warning text-light" style="cursor:pointer; color:white;">
                    <a href="/m/edit-member/?member_id=${
                      member.member_id
                    }" style="cursor:pointer; color:white;">Edit</a>
                </span>
                <span class="badge bg-danger text-light delete-btn" style="cursor:pointer; color:white;" data-member-id="${
                  member.member_id
                }">
                    Delete
                </span>
...
function filterMembers(searchTerm) {
  if (searchTerm === "") {
    ...
    window.BNI.state.filteredMembers = window.BNI.state.allMembers.filter(
      (member) => {
        const fullName =
          `${member.member_first_name} ${member.member_last_name}`.toLowerCase();
  ...
  displayMembers(
    window.BNI.state.filteredMembers.slice(
...
function setupPagination(totalMembers) {
  ...
  // Previous button
  const prevPage = createPaginationButton(
    "Previous",
    window.BNI.state.currentPage === 1
  );
  prevPage.onclick = () => {
  ...
  for (let i = 1; i <= totalPages; i++) {
    const pageItem = createPaginationButton(
      i.toString(),
      false,
  ...
  // Next button
  const nextPage = createPaginationButton(
    "Next",
    window.BNI.state.currentPage === totalPages
  );
  nextPage.onclick = () => {
...
function updateEntriesText(totalMembers) {
  ...
  const start = window.BNI.state.showingAllEntries
    ? 1
    : (window.BNI.state.currentPage - 1) * window.BNI.state.entriesPerPage + 1;
  const end = window.BNI.state.showingAllEntries
    ? totalMembers
    : Math.min(
        window.BNI.state.currentPage * window.BNI.state.entriesPerPage,
        totalMembers
  ...
  if (result.isConfirmed) {
    try {
      ...
      const response = await fetch(
        `https://bni-data-backend.onrender.com/api/deleteMember/${member_id}`,
        {
          method: "PUT",
        }
      );
      ...
      if (response.ok) {
        ...
        document
          .querySelector(`[data-member-id="${member_id}"]`)
          .closest("tr")
          .remove();
      } else {
...
async function updateMemberCounts() {
  try {
    ...
    const activeMembersElement = document.getElementById(
      "active-members-count"
    );
    const inactiveMembersElement = document.getElementById(
...
function sortMembers(columnIndex, ascending) {
  ...
  members.sort((a, b) => {
    ...
    switch (columnIndex) {
      ...
      case 4: // Chapter
        aValue = (
          window.BNI.state.chaptersMap[a.chapter_id] || ""
        ).toLowerCase();
        bValue = (
          window.BNI.state.chaptersMap[b.chapter_id] || ""
        ).toLowerCase();
        break;
