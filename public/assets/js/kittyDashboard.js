...
async function fetchPayments() {
  try {
    ...
    const [regions, chapters, kittyPayments, expenses, orders, transactions, members, credits, bankOrders, activeBill, writeOffs] = await Promise.all([ 
      ...
      fetch('http://backend.bninewdelhi.com/api/getAllKittyPayments').then(res => res.json()),
      fetch('http://backend.bninewdelhi.com/api/allExpenses').then(res => res.json()),
      ...
      fetch('http://backend.bninewdelhi.com/api/getAllMemberWriteOff').then(res => res.json())
    ]);
...
async function updateTotalKittyAmount() {
  try {
    // Get kitty payments
    const kittyResponse = await fetch('http://backend.bninewdelhi.com/api/getAllKittyPayments');
