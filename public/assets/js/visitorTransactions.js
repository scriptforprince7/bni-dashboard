...
async function fetchAllOrders() {
    ...
    try {
        ...
        // First find order with matching visitor_id
        const order = allOrders.find(order => order.visitor_id === parseInt(visitorId));
        console.log("Found order for visitor:", order);

        if (order) {
            orderId = order.order_id; // Now this assignment will work
            ...
            console.error('Order not found for visitor_id:', visitorId);
        }
    } catch (error) {
