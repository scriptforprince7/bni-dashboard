            ...
            trainingsToShow.forEach(async (training, index) => {
                ...
                trainingsTableBody.innerHTML += `
                    <tr>
                        <td>${index + 1}</td>
                        <td>
                            <a href="/c/view-training?training_id=${training.training_id}"><b>${training.training_name}</b></a>
                        </td>
                        <td>${venueDisplay}</td>
...
async function openMailDialog(trainingId) {
    ...
    try {
        ...
        if (result.isConfirmed) {
            ...
            try {
                const response = await fetch('http://localhost:5000/api/sendTrainingMails', {
                    method: 'POST',
                    headers: {
