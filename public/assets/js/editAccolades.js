...
async function fetchAccoladeDetails() {
    try {
        showLoader();
        const response = await fetch(`http://backend.bninewdelhi.com/api/getAccolade/${accoladeId}`);
...
const updateAccoladeData = async () => {
    ...
    if (result.isConfirmed) {
        ...
        try {
            showLoader();
            const response = await fetch(`http://backend.bninewdelhi.com/api/updateAccolade/${accoladeId}`, {
                method: 'PUT',
                headers: {
