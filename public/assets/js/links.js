...
function renderLinksTable() {
  ...
  paymentTypes.forEach((item, idx) => {
    ...
    tdQR.style.width = "140px";
    tdQR.innerHTML = `
      <div style="display:flex;flex-direction:column;align-items:center;">
        <img src="${item.qr.replace('size=90x90', 'size=120x120')}" alt="QR" style="border-radius:8px;border:2px solid #d01f2f;box-shadow:0 2px 8px rgba(0,0,0,0.08);margin-bottom:4px;width:110px;height:110px;">
...
async function renderDynamicLinksTable() {
  ...
  const dynamicLinks = [
    {
      type: "Meeting Payments",
      link: `https://bninewdelhi.com/meeting-payment/4/2d4efe39-b134-4187-a5c0-4530125f5248/1?region_id=${regionId}&chapter_id=${chapterId}`,
      qr: `https://api.qrserver.com/v1/create-qr-code/?size=90x90&data=${encodeURIComponent(`https://bninewdelhi.com/meeting-payment/4/2d4efe39-b134-4187-a5c0-4530125f5248/1?region_id=${regionId}&chapter_id=${chapterId}`)}`
    },
    {
      type: "Training Payments",
      link: `https://bninewdelhi.com/training-payments/3/bdbe4592-738e-42b1-ad02-beea957a3f9d/1?region=${encodeURIComponent(regionName)}&chapter=${encodeURIComponent(chapterName)}`,
      qr: `https://api.qrserver.com/v1/create-qr-code/?size=90x90&data=${encodeURIComponent(`https://bninewdelhi.com/training-payments/3/bdbe4592-738e-42b1-ad02-beea957a3f9d/1?region=${encodeURIComponent(regionName)}&chapter=${encodeURIComponent(chapterName)}`)}`
    },
    {
      type: "Visitors Payment",
      link: `https://bninewdelhi.com/visitors-payment/5/726f7bf6-fa74-4883-b1bb-15acaf64dfb1/1?type=refer&region=${encodeURIComponent(regionName)}&chapter=${encodeURIComponent(chapterName)}`,
      qr: `https://api.qrserver.com/v1/create-qr-code/?size=90x90&data=${encodeURIComponent(`https://bninewdelhi.com/visitors-payment/5/726f7bf6-fa74-4883-b1bb-15acaf64dfb1/1?type=refer&region=${encodeURIComponent(regionName)}&chapter=${encodeURIComponent(chapterName)}`)}`
    },
    {
      type: "New Member Payment",
      link: `https://bninewdelhi.com/new-member-payment/1/8c32021b-2918-4282-9de4-56949626eff7/1?region=${encodeURIComponent(regionName)}&chapter=${encodeURIComponent(chapterName)}`,
      qr: `https://api.qrserver.com/v1/create-qr-code/?size=90x90&data=${encodeURIComponent(`https://bninewdelhi.com/new-member-payment/1/8c32021b-2918-4282-9de4-56949626eff7/1?region=${encodeURIComponent(regionName)}&chapter=${encodeURIComponent(chapterName)}`)}`
    }
  ];
...
async function openMemberSelectModal(paymentItem, idx) {
  ...
  Swal.fire({
    ...
    preConfirm: async () => {
      ...
      // Create separate payloads for each member
      const memberPayloads = checked.map(memberId => ({
        memberIds: [memberId], // Send only one member ID per payload
        paymentLink: paymentItem.type === "Meeting Payments" 
          ? `${paymentItem.link}&member_id=${memberId}`
          : paymentItem.link,
      ...
      console.log("Sending payment links email payloads:", memberPayloads);
      ...
      try {
        ...
        for (const payload of memberPayloads) {
          ...
          console.log("API response for member:", payload.memberIds[0], result);
        }

        Swal.fire({
          icon: 'success',
          title: 'Emails Sent!',
          text: 'Payment links have been sent to all selected members.',
          timer: 2000,
