...
function showLoader() {
  ...
  const fetchExpenseDetails = async (expenseId) => {
    try {
      ...
      const response = await fetch(`http://backend.bninewdelhi.com/api/expense/${expenseId}`);
      ...
      // Fetch expense types
      const expenseTypesResponse = await fetch('http://backend.bninewdelhi.com/api/expenseType');
      ...
      // Fetch all vendors
      const vendorsResponse = await fetch('http://backend.bninewdelhi.com/api/getallvendors');
