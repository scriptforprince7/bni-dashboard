...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    try {
        ...
        const response = await fetch(`http://localhost:5000/api/getUniversalLink/${universalLinkId}`);
        if (!response.ok) throw new Error('Error fetching universal link details');
        
        const universalLink = await response.json();

        const gatewayResponse = await fetch('http://localhost:5000/api/paymentGateway');
...
const updateLinkData = async () => {
    ...
    if (result.isConfirmed) {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/updateUniversalLink/${universalLinkId}`, {
                method: 'PUT',
                headers: {
