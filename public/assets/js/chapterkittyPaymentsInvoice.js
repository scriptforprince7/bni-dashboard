...
function showLoader() {
  ...
  document.addEventListener("DOMContentLoaded", async function() {
    ...
    // Get user's email and login type
    ...
    try {
      ...
      // Find matching chapter and region for the user
      let matchingChapter = null;
      let matchingRegion = null;
  
      if (userEmail && loginType === 'chapter') {
        // Find the chapter that matches the user's email
        ...
        if (matchingChapter) {
          ...
          matchingRegion = regions.find(region => region.region_id === matchingChapter.region_id);
          selectedRegionId = matchingRegion.region_id;
      ...
      // If we found a matching chapter and region, auto-select and disable the dropdowns
      if (matchingRegion && matchingChapter) {
        ...
        // Update member dropdown for the selected chapter
        updateMemberDropdown(selectedChapterId, members);

        // --- RE-ADDED: Fetch kitty bill data automatically for meeting payments ---
        if (universalLinkId === "4") {
          chapterSelectionMessage.style.display = "none";
          paymentTypeOptions.style.display = "block";
          if (kittyBillParticulars) kittyBillParticulars.style.display = "block";

          console.log("🔄 Auto-select: Region and Chapter set", {
            region: matchingRegion.region_name,
            chapter: matchingChapter.chapter_name,
            selectedRegionId,
            selectedChapterId
          });
          console.log("🔄 Auto-select: Fetching Kitty Bill Data for chapter:", selectedChapterId);
          fetchKittyBillData(selectedChapterId);
        }
      } else {
      ...
      function updateChapterDropdown(regionId, allChapters) {
        ...
        filteredChapters.forEach(chapter => {
          ...
          chapterLink.addEventListener("click", async function() {
            ...
            // Hide kitty bill info section ONLY on manual chapter change
