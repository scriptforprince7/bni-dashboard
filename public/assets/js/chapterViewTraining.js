document.addEventListener("DOMContentLoaded", async () => {
    ...
    async function checkAttendanceStatus() {
      try {
        const response = await fetch('http://backend.bninewdelhi.com/api/allCheckins');
        if (!response.ok) {
  ...
  const populateDropdown = (dropdown, data, valueField, textField, defaultText) => {
    ...
    try {
      ...
      // Get login type and determine chapter access
      ...
      if (loginType === 'ro_admin') {
          ...
          if (!chapterEmail || !chapterId) {
              throw new Error('No chapter access details found for admin');
          }
          console.log("👤 Admin accessing chapter:", { chapterEmail, chapterId });
      } else {
          // For regular chapter access, get email from token
          chapterEmail = getUserEmail();
          if (!chapterEmail) {
              throw new Error('No chapter email found in token');
          }
      }
      ...
      const [
          ...
          fetch(`http://backend.bninewdelhi.com/api/getTrainingOrder/${training_id}`),
      ...
      // If chapterId not already set (for regular chapter login), find it from chapters data
      if (!chapterId) {
          const loggedInChapter = chapters.find(chapter => chapter.email_id === chapterEmail);
          if (!loggedInChapter) {
              throw new Error('Chapter not found for the logged-in email');
          }
          chapterId = loggedInChapter.chapter_id;
      }
      console.log("🏢 Accessing chapter ID:", chapterId);

      // Filter orders for the accessed chapter
      const chapterOrders = allOrders.filter(order => order.chapter_id === parseInt(chapterId));
      console.log("📦 Filtered orders for chapter:", chapterOrders);

      // Filter transactions related to chapter orders
      const transactions = allTransactions.filter(transaction =>
          chapterOrders.some(order => order.order_id === transaction.order_id)
      );
  ...
  updateRegistrationCount();
      ...
      tableBody.addEventListener("click", async (event) => {
        ...
        if (target.classList.contains("mark_attendence")) {
          try {
            ...
            const trainingResponse = await fetch('http://backend.bninewdelhi.com/api/alltrainings');
            ...
            const memberResponse = await fetch('http://backend.bninewdelhi.com/api/members');
            ...
            if (matchingMember && matchingMember.member_photo) {
                ...
                const photoUrl = `http://backend.bninewdelhi.com/uploads/memberLogos/${photoFileName}`;
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    fetch("http://backend.bninewdelhi.com/api/markAttendence", {
                        method: "POST",
                        headers: {
                                ...
                                fetch('http://backend.bninewdelhi.com/api/allTransactions')
                                    ...
                                    .then(transactions => {
                                        ...
                                        if (transaction) {
                                            ...
                                            fetch('http://backend.bninewdelhi.com/api/send-qr-code', {
                                                method: 'POST',
                                                headers: {
                                                    'Content-Type': 'application/json',
                                                },
      ...
      showToast('error', error.message || 'Failed to load training data');
    } finally {
      hideLoader();
    }
  });
            ...
            async (decodedText, decodedResult) => {
              ...
              if (result.isConfirmed) {
                  try {
                      ...
                      const response = await fetch("http://backend.bninewdelhi.com/api/verify-qr-code", {
                          method: "POST",
                          headers: {
                              "Content-Type": "application/json",
  ...
  async function fetchTrainingDetails() {
    try {
      ...
      const [trainingResponse, hotelsResponse] = await Promise.all([
        fetch(`http://backend.bninewdelhi.com/api/getTraining/${training_id}`),
