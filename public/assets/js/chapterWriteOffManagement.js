...
function showLoader() {
  ...
  function viewMembers(memberIds) {
    const memberDetails = memberIds.map(id => {
      ...
      return member
        ? {
            name: member.member_first_name,
            phone: member.member_phone_number,
            status: member.member_status
          }
        : { name: id, phone: "N/A", status: "N/A" };
    });
  
    let tableContent =
      "<table><tr><th>Index</th><th>Member Name</th><th>Phone Number</th><th>Status</th></tr>";
    memberDetails.forEach((detail, index) => {
      tableContent += `<tr style="border: 1px solid #ddd; margin: 5px 0; text-align: center;"><td>${index +
        1}</td><td style="display: flex; align-items: center;"><img src="https://cdn-icons-png.flaticon.com/512/194/194828.png" alt="Member" style="width: 20px; height: 20px; border-radius: 50%; margin-right: 5px;">${detail.name}</td><td>${detail.phone}</td><td>${detail.status}</td></tr>`;
    });
    tableContent += "</table>";
  
    Swal.fire({
      title: "Members that writeoff are:",
      html: tableContent
    });
  }
