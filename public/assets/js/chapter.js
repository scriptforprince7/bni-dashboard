...
function getMemberCountForChapter(chapterId) {
  ...
  return window.BNI.state.allMembers.filter(
...
function getRegionNameById(regionId) {
  ...
  const region = window.BNI.state.allRegions.find(
...
function updateRunningChaptersCount() {
  const runningCount = window.BNI.state.filteredChapters.filter(
    (chapter) => chapter.chapter_status === "running"
  ).length;
  const runningChaptersElement = document.getElementById(
    "running-chapters-count"
  );
...
const displayChapters = (chapters) => {
  ...
  const end = showingAll
    ? chapters.length
    : Math.min(start + entriesPerPage, chapters.length);

  // Update showing entries text
  document.getElementById("showingStart").textContent =
    chapters.length > 0 ? start + 1 : 0;
  ...
  chaptersToShow.forEach((chapter, index) => {
    ...
    const cleanStatus = chapter.chapter_status
      .replace(/['"]+/g, "")
      .trim()
      .toLowerCase();
    ...
    console.log(
      `Running count: ${active_total}, Inactive count: ${inactive_total}`
    );
    ...
    row.innerHTML = `
            ...
            <td style="border: 1px solid grey;"><b>₹${
              chapter.available_fund
            }</b></td>
            <td style="border: 1px solid grey;">${
              chapter.chapter_meeting_day || "N/A"
            }</td>
            <td style="border: 1px solid grey;"><b>₹${
              chapter.chapter_kitty_fees || "0"
            }</b></td>
            <td style="border: 1px solid grey;"><b>₹${
              chapter.chapter_visitor_fees || "0"
            }</b></td>
            <td style="border: 1px solid grey;"><b>${
              chapter.kitty_billing_frequency || "N/A"
            }</b></td>
            <td style="border: 1px solid grey;">
                <span class="badge bg-${
                  cleanStatus === "running" ? "success" : "danger"
                }">
                    ${
...
function populateFilters() {
  ...
  const uniqueRegions = [
    ...new Set(
      window.BNI.state.allChapters.map((chapter) => {
        const regionName = getRegionNameById(chapter.region_id);
        console.log(
          `📍 Found region: ${regionName} for chapter: ${chapter.chapter_name}`
        );
        return regionName;
      })
    ),
...
function applyFilters() {
  ...
  window.BNI.state.filteredChapters = window.BNI.state.allChapters.filter(
    (chapter) => {
      // Get clean values
      ...
      const matchesMeetingDay =
        meetingDay === "all" || chapterMeetingDay === meetingDay;
      const matchesStatus =
        chapterStatus === "all" || cleanStatus === chapterStatus.toLowerCase();
      const matchesRegion =
        !region ||
        region === "all" ||
        getRegionNameById(chapter.region_id) === region;

      console.log(`Checking ${chapter.chapter_name}:`, {
        meeting: {
          has: chapterMeetingDay,
          want: meetingDay,
          matches: matchesMeetingDay,
        },
        status: {
          has: cleanStatus,
          want: chapterStatus,
        ...
        region: {
          has: getRegionNameById(chapter.region_id),
          want: region,
  ...
  console.log(
    `Found ${window.BNI.state.filteredChapters.length} matching chapters`
  );
...
document.getElementById("region-filter")?.addEventListener("click", (event) => {
  if (event.target.classList.contains("dropdown-item")) {
    ...
    event.target
      ...
      .querySelector(
        ".dropdown-toggle"
      ).textContent = `Region: ${event.target.textContent}`;
  }
});

document
  .getElementById("meeting-day-filter")
  ?.addEventListener("click", (event) => {
    if (event.target.classList.contains("dropdown-item")) {
      ...
      event.target
        ...
        .querySelector(
          ".dropdown-toggle"
        ).textContent = `Meeting Day: ${event.target.textContent}`;
    }
  });

document
  .getElementById("chapter-status-filter")
  ?.addEventListener("click", (event) => {
    if (event.target.classList.contains("dropdown-item")) {
      ...
      event.target
        ...
        .querySelector(
          ".dropdown-toggle"
        ).textContent = `Status: ${event.target.textContent}`;
    }
  });
...
document.getElementById("reset-filters-btn")?.addEventListener("click", () => {
  ...
  document
    ...
    .closest(".dropdown")
    .querySelector(".dropdown-toggle").textContent = "Region";
  document
    .querySelector("#meeting-day-filter")
    .closest(".dropdown")
    .querySelector(".dropdown-toggle").textContent = "Meeting Day";
  document
    .querySelector("#chapter-status-filter")
    .closest(".dropdown")
    .querySelector(".dropdown-toggle").textContent = "Chapter Status";
...
document
  .getElementById("sort-name-filter")
  ?.addEventListener("click", (event) => {
    if (event.target.classList.contains("sort-option")) {
      ...
      console.log(
        "✅ Sorting complete. First few chapters:",
      ...
      event.target
        .closest(".dropdown")
        .querySelector(".dropdown-toggle").textContent = `Sort By Name: ${
...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    const [chaptersResponse, regionsResponse, membersResponse] =
      await Promise.all([
    ...
    if (!regionsResponse.ok)
      throw new Error(`Regions fetch failed: ${regionsResponse.status}`);
...
document
  ...
  ?.addEventListener("input", function () {
    ...
    window.BNI.state.filteredChapters = window.BNI.state.allChapters.filter(
      (chapter) => chapter.chapter_name.toLowerCase().includes(searchTerm)
    );
      ...
      if (result.isConfirmed) {
        try {
          ...
          const response = await fetch(
            `https://bni-data-backend.onrender.com/api/deleteChapter/${chapterId}`,
            {
              method: "PUT",
            }
          );
...
function sortChapters(columnIndex, ascending = true) {
  console.log(
    `🔄 Starting sort on column ${columnIndex}, ascending: ${ascending}`
  );
  ...
  console.log(
    `✅ Sorting complete. First few entries:`,
