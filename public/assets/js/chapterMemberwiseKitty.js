document.addEventListener("DOMContentLoaded", () => {
 ...
 async function fetchMemberWiseKitty() {
   try {
     ...
     const membersWithPending = members
       .filter(member => {
         // Only show entries for logged in chapter
         return member.chapter_id === loggedInChapter.chapter_id;
       })
       .map(member => {
         const memberCredit = credits.find(credit => credit.member_id === member.member_id);
         ...
         const memberOrders = orders.filter(o => 
           o.customer_id === member.member_id && 
         ...
         console.log(`💰 Payment details for member ${member.member_first_name}:`, {
           ...
           pendingAmount: formatInIndianStyle(member.meeting_payable_amount)
         });
       ...
       const tableContent = membersWithPending
         .map((member, index) => {
           ...
           if (kittyBill) {
             ...
             // Check for meeting payable amount directly from members data
             const memberData = members.find(m => m.member_id === member.memberId);
             console.log(`🔍 Checking meeting payable amount for ${member.memberName}:`, {
               memberId: member.memberId,
               payableAmount: memberData ? memberData.meeting_payable_amount : 'No member data found'
             });
             
             // Combined condition: due date must have passed AND meeting_payable_amount > 0
             if (today > dueDate && memberData && parseFloat(memberData.meeting_payable_amount) > 0) {
               console.log(`⚠️ Late payment detected for ${member.memberName} (chapter ${member.chapterName}) - Due date has passed AND has pending amount`);
               latePaymentCount = 1;
             } else {
               console.log(`✅ Payment on time or no pending amount for ${member.memberName} (chapter ${member.chapterName})`);
             }
           } else {
           ...
           return `
             <tr>
               ...
               <td style="border: 1px solid lightgrey; text-align: center;"><strong>₹ ${formatInIndianStyle(member.totalPaid || 0)}</strong></td>
               <td style="border: 1px solid lightgrey; text-align: center;"><strong>₹ ${formatInIndianStyle(member.creditAmount || 0)}</strong></td>
 ...
 // Calculate total from displayed table values
 const pendingBalanceCells = document.querySelectorAll('.pending-balance-cell');
 let totalDisplayedAmount = 0;
 
 pendingBalanceCells.forEach(cell => {
   const pendingAmount = parseFloat(cell.getAttribute('data-pending-amount')) || 0;
   totalDisplayedAmount += pendingAmount;
 });
 
 console.log('💰 Total Member Kitty Amount (from displayed table):', {
   chapterId: loggedInChapter.chapter_id,
   chapterName: loggedInChapter.chapter_name,
   totalDisplayedAmount: totalDisplayedAmount,
   memberCount: pendingBalanceCells.length
 });
 
 // Update the total kitty amount display with sum of displayed values
 const totalKittyElement = document.getElementById('totalKittyDetails');
 if (totalKittyElement) {
   totalKittyElement.textContent = formatInIndianStyle(totalDisplayedAmount);
   console.log('✅ Updated total kitty amount in UI:', formatInIndianStyle(totalDisplayedAmount));
 } else {
   console.error('❌ Element with id="totalKittyDetails" not found');
 }
