...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (generateBulkPdfBtn) {
        generateBulkPdfBtn.addEventListener('click', async function() {
            try {
                ...
                if (selectedIds.length === 0) {
                    Swal.fire({
                        icon: 'warning',
                        title: 'No Selection',
                        text: 'Please select at least one invoice to generate PDF'
                    });
                    return;
                }
                ...
                // Show loading state
                generateBulkPdfBtn.disabled = true;
                ...
                const a = document.createElement('a');
                a.href = url;
                a.download = `bulk-einvoices-${new Date().toISOString().split('T')[0]}.pdf`;
                document.body.appendChild(a);
                a.click();
                ...
                Swal.fire({
                    icon: 'success',
                    title: 'Success',
                    text: 'Bulk PDF generated successfully!'
                });
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'Failed to generate bulk PDF. Please try again.'
                });
            } finally {
                // Reset button state
                generateBulkPdfBtn.disabled = false;
                generateBulkPdfBtn.innerHTML = '<i class="ti ti-file-download me-1"></i> Generate Bulk PDF';
            }
        });
    }
});
